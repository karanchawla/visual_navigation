	...
	def extractImages(self, bagfile, topicname, outputdir, camera_file, compressed):
		...
		with open(camera_file, "w") as f:
			...
			for ts in self.timestamps:
				f.write("data/IMG/" + str(index) + ".jpg" + "," + str(ts) + "," + str(0))
				f.write("\n")
				index += 1
		bag.close()
...
if __name__ == "__main__":
	...
	extract_files.extractImages(bagfile, image_topic, outputdir, CAMERA_TIME_FILE, compressed)
